...
Viewer HTML から localhost WebSocket を通して、Windows DirectInput の FFB effect を制御する Windows 専用アプリ。ブラウザ、Momo、Raspberry Pi はホイールへ直接出力しない。
...
`build-ffb-bridge-zip.ps1` は .NET Runtime を含む `win-x64` 用 ZIP を生成する。
展開後に `MomoFpvFfbBridge.exe` を起動すると、Bridge、DirectInput デバイス検出、
Relay が配信する Viewer を開くためのランチャーが1つのウィンドウで動作する。Python は不要である。

```powershell
cd C:\src\momo-fpv-viewer\tools\ffb-bridge
.\build-ffb-bridge-zip.ps1
```

GUIの `Relay endpoint` には、RelayのPilot endpointを `127.0.0.1:8090` の形式で入力する。
Relayが別PCにある場合だけ、そのPCのIPまたはホスト名とポートを入力する。
`Relay device` には、そのRelayが管理する車体sourceを `11.3` の形式で入力する。複数sourceのRelayでは
...
- `Stop FFB`: 現在の effect を即時停止する。Viewerが Drive On のままなら次の制御更新で再開するため、緊急時はViewer側もDrive Offにする。
...
Input 設定画面から `Viewer を開く` を使う。FFB 有効時は、選択したプロファイルの `ffbEnabled`、friction/damper 設定、`ffbUrl` が Viewer URL と同じ Relay origin の localStorage に保存される。ランチャーの `Open Viewer` から開いても同じ設定が反映される。

Relay Variant では、Relay Viewer と gamepad 設定画面をこの Viewer 正本から同期してから使う。公開された GitHub Pages Viewer を FFB Bridge に許可しない。
